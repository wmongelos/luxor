    ...
    class FormaPagoNegocios
    {
        public DataTable GetData()
        {
            ...
            try
            {
                ...
                using (var context = new dbLuxorEntities())
                    Data = tools.ToDataTable(context.Medios_Pagos.Where(x => x.Borrado == false).OrderBy(x => x.Descripcion).ToList());
            }
            catch { return null; }
        ...
        public String Save(Int32 Id, String Descripcion)
        {
            try
            {
                using (var context = new dbLuxorEntities())
                {
                    Medios_Pagos F_Pago = Id > 0 ? context.Medios_Pagos.Where(x => x.Id == Id).FirstOrDefault() : new Medios_Pagos();
                    F_Pago.Descripcion = Descripcion;


                    if (F_Pago.Id == 0)
                        context.Medios_Pagos.Add(F_Pago);
        ...
        public String Delete(Int32 Id) 
        {
            try
            {
                using (var context = new dbLuxorEntities())
                {
                    context.Medios_Pagos.Where(x => x.Id == Id).FirstOrDefault().Borrado = true;
                    context.SaveChanges();
                }
            }
